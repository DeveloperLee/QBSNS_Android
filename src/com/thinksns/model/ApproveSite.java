...
public class ApproveSite extends SociaxItem {
	...
	public ApproveSite (JSONObject siteData) throws SiteDataInvalidException{
		try {
			...
			this.setDescription(siteData.has("description")?siteData.getString("description"):"");
	...
	public ApproveSite (JSONObject siteData) throws SiteDataInvalidException{
		try {
			...
			this.setStatus_mtime(siteData.has("status_mtime")?siteData.getString("status_mtime"):"");
			this.setDenied_reason(siteData.has("denied_reason")?siteData.getString("denied_reason"):"");
			this.setIsInUsed(siteData.has("isused")?siteData.getInt("isused"):0);
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			throw new SiteDataInvalidException(e.getMessage());
	...
	public String toString() {
		return "ApproveSite [site_id=" + site_id + ", name=" + name + ", url="
				+ url + ", logo=" + logo + ", description=" + description
				+ ", uid=" + uid + ", email=" + email + ", phone=" + phone
				+ ", ctime=" + ctime + ", status=" + status + ", status_mtime="
				+ status_mtime + ", denied_reason=" + denied_reason
				+ ", isInUsed=" + isInUsed + "]";
	}
